...
HRESULT CRenderer::Init(HWND hWnd, bool bWindow)
{
	...
	m_pD3DDevice->SetRenderState(D3DRS_LIGHTING, TRUE);						// ライトON
...
{
    // バックバッファ＆Ｚバッファのクリア
    m_pD3DDevice->Clear(0, NULL, (D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER), D3DCOLOR_RGBA(0, 0, 255, 255), 1.0f, 0);
    // Direct3Dによる描画の開始
    if(SUCCEEDED(m_pD3DDevice->BeginScene()))
    {
		CManager::GetGamemode()->Draw();
        // ポリゴンの描画処理
        CObject::DrawAll();
