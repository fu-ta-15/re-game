...
HRESULT InitGauge(void)
{
	...
	// 変数の初期化
	g_Gauge.PerfectStart = GAUGE_START;
	g_Gauge.PerfectGole = GAUGE_GOLE;
	g_Gauge.PerfectColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	...
	for (int nCunt = 0; nCunt < GAUGE_TYPE; nCunt++)
	{
		if (nCunt == 0)
		{
			...
			//頂点カラーの設定・赤・緑・青
			pVtx[0].col = g_Gauge.PerfectColor;
			pVtx[1].col = g_Gauge.PerfectColor;
			pVtx[2].col = g_Gauge.PerfectColor;
			pVtx[3].col = g_Gauge.PerfectColor;
		}
		else if (nCunt == 1)
		{
			pVtx[0].pos = D3DXVECTOR3(g_Gauge.PerfectStart, SCREEN_HEIGHT, 0.0f);
			pVtx[1].pos = D3DXVECTOR3(g_Gauge.PerfectStart, GAUGE_HEITH, 0.0f);
			pVtx[2].pos = D3DXVECTOR3(g_Gauge.PerfectGole, SCREEN_HEIGHT, 0.0f);
			pVtx[3].pos = D3DXVECTOR3(g_Gauge.PerfectGole, GAUGE_HEITH, 0.0f);
...
void UpdateGauge(void)
{
	...
	case ACTION_MODE_ATTACK:
		g_Gauge.PerfectStart = 620.0f;
		g_Gauge.PerfectGole = 660.0f;
		break;
	case ACTION_MODE_DIG:
		g_Gauge.PerfectStart = 625.0f;
		g_Gauge.PerfectGole = 655.0f;
		break;
	default:
		break;
	}
