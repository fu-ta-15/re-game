...
void ActionCondributor(void)
{
	...
	{
		if (g_Action.pos.x < pGauge->PerfectGole && g_Action.pos.x > pGauge->PerfectStart)
		{
			switch (g_Action.Amode)
			{
			case ACTION_MODE_ATTACK:	// 攻撃
		...
		else if (g_Action.pos.x > pGauge->PerfectGole || g_Action.pos.x < pGauge->PerfectStart)
		{
			switch (g_Action.Amode)
			{
			case ACTION_MODE_ATTACK:	// 攻撃
