...
HRESULT InitPause(void)
{
	...
	for (int nCuntPause = 0; nCuntPause < MAX_MENU; nCuntPause++)
	{
		...
		{
			//頂点座標の設定(X座標・Y座標・Z座標(2Dは0固定)右回りで描画）
			pVtx[0].pos = D3DXVECTOR3(230.0f, 100.0f + (nCuntPause * 100), 0.0f);
			pVtx[1].pos = D3DXVECTOR3(230.0f, 0.0f + (nCuntPause * 100), 0.0f);
			pVtx[2].pos = D3DXVECTOR3(440.0f, 100.0f + (nCuntPause * 100), 0.0f);
			pVtx[3].pos = D3DXVECTOR3(440.0f, 0.0f + (nCuntPause * 100), 0.0f);
...
void UninitPause(void)
{
	...
	StopSound(SOUND_LABEL_SE_PAUSE0);	//BGM
	StopSound(SOUND_LABEL_SE_PAUSE1);	//BGM
}
...
void UpdatePause(void)
{
	...
	if (GetKeyboardTrigger(DIK_S) == true || pPad[JPTRIGGER_UP] == true)
	{
		g_Pause.nSelectold = g_Pause.nSelectCounter;

		g_Pause.nSelectCounter += 1;
		PlaySound(SOUND_LABEL_SE_PAUSE0);

	}
	else if (GetKeyboardTrigger(DIK_W) == true || pPad[JPTRIGGER_DOWN] == true)
	{
		g_Pause.nSelectold = g_Pause.nSelectCounter;
	...
	if (GetKeyboardTrigger(DIK_RETURN) == true || GetKeypadTrigger(3) == true)
	{
		...
		PlaySound(SOUND_LABEL_SE_PAUSE1);
	}
